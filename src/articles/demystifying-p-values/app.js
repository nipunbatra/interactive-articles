...
let state = {
  scenario: 'smartPills',
  bucketMixed: false,
  manualGaps: [], // stores all manually drawn gaps
  allCalculated: false,
  allResplits: [],
  observedGap: 0,
  animationProgress: 0,
...
function drawSingleSplit() {
  ...
  elements.manualResultBox.innerHTML = `
    <div class="flex-cols">
      ...
      <div><strong>Gap:</strong> <span style="color:var(--extreme);font-weight:bold;">${gap.toFixed(1)}</span></div>
    </div>
  `;
...
elements.btnDrawMany.addEventListener('click', async () => {
  ...
  for (let i = 0; i < 10; i++) {
    drawSingleSplit();
    await new Promise(r => setTimeout(r, 100)); // 100ms delay for animation effect
  }
...
function drawManualCanvas() {
  ...
  state.manualGaps.forEach((gap, i) => {
    const x = getX(gap);
    // Add vertical jitter based on index to prevent perfect overlap
...
function animateHistogram() {
  state.animationProgress += 0.025; // speed
...
function finishCalculation() {
  ...
  let explanation = "";
  if (pVal < 0.05) {
    explanation = `Because this p-value is very small (less than the typical 0.05 threshold), it's highly unlikely we'd see a gap this large by pure chance. We have strong evidence that the treatment actually had an effect!`;
  } else {
    explanation = `Because this p-value is large, a gap of ${state.observedGap.toFixed(1)} happens quite often by pure chance (about ${(pVal*100).toFixed(1)}% of the time). We don't have enough evidence to say the treatment worked. It might just be random noise.`;
  }
